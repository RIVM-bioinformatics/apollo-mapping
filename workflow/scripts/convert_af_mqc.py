...
def main(args):
    ...
    df_clean_pivoted = pd.pivot_table(df_clean, index="Sample", columns="AF", values=['Count'], fill_value=0)
    df_clean_pivoted.columns = ['_'.join(col).strip() for col in df_clean_pivoted.columns.values]
    df_clean_pivoted.reset_index(inplace=True)
    df_clean_pivoted.columns = df_clean_pivoted.columns.map({"Sample": "Sample", "Count_0.500": "Heterozygous", "Count_1.00": "Homozygous"})
    # Calculate ratio heterozygous (mono-allelic) variants from total number of (mono-allelic) variants
    df_clean_pivoted['Heterozygosity_ratio'] = df_clean_pivoted['Heterozygous'] / (df_clean_pivoted['Heterozygous'] + df_clean_pivoted['Homozygous'])
