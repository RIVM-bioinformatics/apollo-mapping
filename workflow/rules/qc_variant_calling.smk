...
rule count_allelefreq_multiallelic:
    ...
    message:
        "Writing allele frequency and multi-allelic status of variants to table for {wildcards.sample}"
    conda:
        "../envs/gatk_picard.yaml"
    container:
        "docker://broadinstitute/gatk:4.4.0.0"
...
rule combine_allelefreq_multiallelic:
    input:
        expand(
            OUT + "/qc_variant_calling/count_allelefreq_multiallelic/{sample}.tsv",
            sample=SAMPLES,
        ),
    output:
...
rule variant_evaluation:
    ...
    message:
        "Variant evaluation for {wildcards.sample}"
    conda:
        "../envs/gatk_picard.yaml"
    container:
        "docker://broadinstitute/gatk:4.4.0.0"
